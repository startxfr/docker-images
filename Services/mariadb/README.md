...
## Running from docker registry
	...
	docker run -d -v /var/lib/mysql -v /var/log/mysql --name mariadb-data startx/sv-mariadb:centos7  echo "Data container for mariadb"
	docker run -d -p 3306:3306 --volumes-from mariadb-data --name="mariadb" startx/sv-mariadb:centos7
	# when used in a linked container
	docker run -d --name="mariadb" startx/sv-mariadb:centos7
	docker run -d --name="php" --link mariadb:mariadb startx/sv-php:centos7
...
### Running local image
	...
	docker run -d -p 3306:3306 --name="mariadb" sv-mariadb:centos7
