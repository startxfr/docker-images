...
service:
  ...
  container_name: "centos6-service-ooconv"
  environment:
    CONTAINER_TYPE: "service"
    CONTAINER_SERVICE: "ooconv"
    CONTAINER_INSTANCE: "centos6-service-ooconv"
  volumes:
    - "/tmp/container-centos6/logs/ooconv:/logs"
```
...
4. Run this container 
  1. Interactively with `docker run -p 2002:2002 -v /logs -it sv-ooconv-centos6`. If you add a second parameter (like `/bin/bash`) to will run this command instead of the default entrypoint. Usefull to interact with this container (ex: `/bin/bash`, `/bin/ps -a`, `/bin/df -h`,...) 
  2. As a daemon with `docker run -p 2002:2002 -v /logs -d sv-ooconv-centos6`
