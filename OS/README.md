...
s2i build . startx/fedora:27 startx-bash-myapp
...
|---------------------------|----------|-----------------|--------------------------------------------------------------------------|
| SX_VERSION                | `string` | `27`            | container version
...
* Get version 27 of this container OS source code
...
2. Build the container using `docker build -t startx-fedora27 .`
3. Run this container 
   - Interactively with `docker run -it startx-fedora27`. If you add a second parameter (like `/bin/bash`) to will run this command instead of the default entrypoint. Usefull to interact with this container (ex: `/bin/bash`, `/bin/ps -a`, `/bin/df -h`,...) 
   - As a daemon with `docker run -d startx-fedora27`
